...
def CLICK(channel):
	...
	conn = pyodbc.connect("DRIVER={ms-sql};SERVER=moldex-sl.moldex.com;PORT=1433;DATABASE=wagodb;UID=sa;PWD=moldex")
	cursor = conn.execute("{call dbo.Click(?, ?)}", machineid, datetime.datetime.now())
	conn.commit()
	conn.close()
...
class BarcodeThread(BaseThread):
    def run(self):
        ...
        while True:
			line = raw_input("Please Scan A Barcode:")      
			WhatWasScanned(line)
			CheckMachineState()       
